...
class ExistingFile(Openable):
	__slots__ = ("parent", "idx", "_path", "_stat", "_dirty", "flags", "password", "extras")
	...
	def _dirtyGuard(self):
		if self.dirty:
			raise RuntimeError("Doing this op to a dirty file will result in a crash")

	def _openGuard(self):
		if self.ptr:
			raise RuntimeError("Doing this op to a opened file will result in a crash")
	...
	def unchange(self) -> None:
		unchange(self.parent.ptr, self.idx)
		self._dirty = False
